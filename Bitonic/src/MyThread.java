...
public class MyThread extends AbstractMyThread {

	public MyThread(CyclicBarrier barrier, int Id, int cubeSize) {
		myId = Id;
	...
	public void run() {
		...
		while (counter < cubeSize) {
			...
			while (secondCounter >= 0) {
				...
				final boolean temperiger = temper < (int) (Math.pow(2, counter + 1));
				if (temper < (int) (Math.pow(2, counter + 1))) {
					if (myId < otherThread.myId) {
						tmp = mergesortEven(otherThread.numbers);
					} else {
						tmp = mergesortUneven(otherThread.numbers);
					}
				} else {
					if (myId < otherThread.myId) {
						tmp = mergesortUneven(otherThread.numbers);
